# Security, Storage, and Firmware Update

Use these notes for cryptography, TLS, secure storage, filesystems, flash persistence, MCUboot, MCUmgr, hawkBit, TF-M, and security updates.
...
## Secure storage, filesystems, and persistence
...
## MCUboot, TF-M, MCUmgr, and hawkBit
...
### MCUmgr Bluetooth permissions — `migration-4.0`

`MCUMGR_TRANSPORT_BT_AUTHEN` is replaced by the `CONFIG_MCUMGR_TRANSPORT_BT_PERM_RW` choice. `CONFIG_MCUMGR_TRANSPORT_BT_PERM_RW_AUTHEN` requests authenticated read/write access; select `CONFIG_MCUMGR_TRANSPORT_BT_PERM_RW` to remove the default authentication requirement.
