...
Handlers and services added to `Router` or `MethodRouter` must satisfy `Sync` in axum 0.8.0 (batch `0.8-guide`). This includes the routed value itself and any captured state that determines whether it is safe to share between threads. Replace non-`Sync` captures or service internals before registering them.
