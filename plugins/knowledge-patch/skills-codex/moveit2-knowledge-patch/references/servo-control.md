...
Servo scales commanded velocities near singularities, self-collisions, and
world collisions while enforcing joint position and velocity limits.
Collision checking and command smoothing are independently controlled by
...
`robot_description_kinematics` parameters to `ServoNode`; the resulting
parameter paths are
...
plugin with custom parameters must declare them inside the plugin.

## Realtime thread scheduling

When a realtime kernel is available, the main `ServoNode` loop automatically
attempts `SCHED_FIFO` at priority `40`. This reduces control-loop jitter
without a separate scheduler wrapper.

## Direct C++ interface

Construct `Servo` from a generated parameter listener and planning-scene
monitor. Select a `CommandType`, repeatedly supply `JointJogCommand`,
`TwistCommand`, or `PoseCommand` to `getNextJointState()`, and consume the
returned `KinematicState` with joint names, positions, velocities, and
accelerations.
...
`ServoNode` receives these messages on its parameterized input topics:
...
`std_msgs::msg::Float64MultiArray` on `command_out_topic`. Change the active
