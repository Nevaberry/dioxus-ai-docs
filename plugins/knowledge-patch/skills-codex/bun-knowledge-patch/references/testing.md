...
Use this reference for bun:test APIs, discovery, isolation, concurrency, sharding, coverage, reporters, mocks, snapshots, and external test-runner compatibility.

## Automatically disposable test mocks
...
## Callback-style expected-failure tests
...
## Compile-time type assertions in `bun:test`
...
Debugger breakpoints now resolve in ES modules loaded by `bun test --isolate` or `--parallel`, and inside executables produced by `bun build --compile`.
...
## Exact repeated-value assertions
...
Subtests and inline `test()`/`describe()` calls now run, with `t.plan()`, `t.waitFor()`, `getTestContext()`, mock timers and properties, runtime skip/todo, tags, custom assertions, and callback tests. The programmatic `run()` API returns a Node-compatible `TestsStream`, `expectFailure` is supported, and each test's mock tracker resets automatically.
...
## Initial `node:test` compatibility
...
## Isolated and parallel test files
...
## Named fields in parameterized tests
...
`node:assert` now includes the configurable `Assert` class and native `assert.partialDeepStrictEqual()`.

## Playwright and Vitest compatibility

*Batch: `1.4`.*

Playwright now runs under Bun with `connectOverCDP()`, `playwright test`, TypeScript configuration, UI mode, and Chromium launching on Windows. Vitest also runs under Bun with coverage and both thread and fork pools.

## Successful empty test runs
...
## Test discovery path exclusions
...
## Test output elision outside terminals
...
## Test reset and containment semantics

*Batch: `1.4-2`.*

`jest.resetAllMocks()` and `vi.resetAllMocks()` now discard mock implementations as well as call history; use `clearAllMocks()` to retain implementations. `toContain()` now uses `===`, so `-0` matches `0` and `NaN` does not match itself.

## Test retries and repetitions
...
## Test timeouts terminate synchronous child processes
...
## Timing-aware test distribution

*Batch: `1.4`.*

`--update-timings` records per-file durations, and `--timings=<path>` uses them to balance `--shard` by wall time and make `--parallel` start slow files first. Files sharing imports remain together, and the slowest-first timing file doubles as a slow-test report.

```sh
bun test --timings=timings.json --update-timings
bun test --shard=1/3 --timings=timings.json
bun test --parallel --timings=timings.json
```

## Unhandled test failures
